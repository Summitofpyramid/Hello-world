function [datapoints, data] = getEBMsize(fileName)

%	=======================================
%	Count number of data points in EBM file
%	=======================================
%
%	Version 3.0
...
%   Modified by Chris McNorgan
%    USAGE:
%
%      datapoints = getEBMsize( fileName );
%
%      fileName  : Filename, including path. If omitted a standard get file dialog will appear. 
%      
%      Select an .ebm file that corresponds to a channel (e.g., Fz, Cz, Pz, etc...)
%
%      datapoints: The number of measurements in the recording
%      data: The actual data measurements (as long as we have them, we may
%      as well return them)
...
if nargin<1
	[ filename, filepath ] = uigetfile('*.ebm', 'Select a .EBM data file:');
	if isempty( filename )
		error( ERROR_CANCEL );
	end;
    ...
    %fp = fopen( char(fileName), 'rb', endian );
    ...
    while 1
        ...
        if ( rec == EBM_R_DATA )
            ...
            if recnum==1
                if datapoints>0
                    %if we have the information to preallocate dataTemp...
                    dataTemp=nan(datapoints, 1);
                    dataTemp(1:length(newdata))=newdata;
                else
                    dataTemp=newdata;
                end
                prlen=length(dataTemp);
                ...
                if datapoints>0
                    %find the first NAN, that's where the filldata goes,
                    %then the newdata goes in after that.
                end
                dataTemp=[dataTemp; filldata; newdata];
                prlen=length([filldata; newdata]);
                clear newdata filldata;
            end
        end;
    ...
    fclose( fp );
    datapoints=length(dataTemp);%now we can capture how many data points there are.
    data=dataTemp';
